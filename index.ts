...
export const getSchoolID = async ({ region, level, query }: { region: keyof typeof Region, level: keyof typeof Level, query: string }): Promise<SchoolInfo[]> => {
    if (!Level[level]) throw new Error('지역정보가 올바르지 않습니다')
    if (!Region[region]) throw new Error('교급정보가 올바르지 않습니다')
    return fetch(`https://hcs.eduro.go.kr/v2/searchSchool?lctnScCode=${Region[region] + 1}&schulCrseScCode=${Level[level] + 1}&orgName=${encodeURI(query)}&loginType=school`)
        .then(e => e.json())
        .then(e => {
            if (e.schulList.length === 0) throw new Error("해당하는 학교를 찾을 수 없습니다.")
            return e.schulList
        })
        .catch(e => {
...
export const takeSurvey = async ({
    ...
    return fetch('https://goehcs.eduro.go.kr/registerServey', {
        method: 'POST',
        headers: {
            Authorization: token,
            "Content-Type": "application/json"
